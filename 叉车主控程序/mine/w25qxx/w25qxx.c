...
//读取flash
	u8 Buffer_Read[2000];
void W25QXX_Read_16(u16* Buffer16,u32 ReadAddr,u16 NumByteToRead)
{
	u16 i = 0;
	W25QXX_Read(Buffer_Read,ReadAddr*2,NumByteToRead*2);
	for(i=0;i<NumByteToRead;i++)
	{
		Buffer16[i] = (Buffer_Read[i*2]<<8)+Buffer_Read[i*2+1];
	}
}	
//写入flash
...
void W25QXX_Write_16(u16* Buffer16,u32 WriteAddr,u16 NumByteToWrite)
{
	...
	{
		Buffer_Write[i*2] = Buffer16[i]>>8;
		Buffer_Write[i*2+1] = Buffer16[i];
	}
	W25QXX_Write(Buffer_Write,WriteAddr*2,NumByteToWrite*2);
}
